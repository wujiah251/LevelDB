...
namespace leveldb
{
  ...
  namespace
  {
    ...
    private:
      ...
      // 元素的内存占用并不是LRUHandle对象的大小，Insert()的时候上层调用者
      // 负责传入。
      size_t usage_;
        ...
        void (*deleter)(const Slice &key, void *value))
    {
      ...
      if (capacity_ > 0)
      {
        ...
        LRU_Append(&in_use_, e);
        usage_ += charge;
        // 需要删除可能替换那个节点（如果存在相同的key，不会改写，
        // 而是添加新的节点，把原来的节点从哈希表和链表中删除）
        // 但是其分配的空间不会释放
    ...
    // 的缓存、查询、删除等操作。
    // 其设计是因为每次访问LRUCache都会获得互斥锁，未来提高性能，对缓存做了一个分片
